...
def banner1() -> None:
    print(
        colored("⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀", "red", "on_black", ["bold"])
        + colored("⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀", "green", "on_black", ["bold"])
        + colored("⠀⠀⠀⠀⠀⠀⠀⠀⡖⡄⠀⠀⢀⣀⢤⢤⢤⢤⢤⢤⣄⣄⣄⣠⣀⣀⣀⢀", "light_cyan", "on_black", ["bold"])
    )
    print(
        colored("⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀", "red", "on_black", ["bold"])
        + colored("⠀⠀⢀⣀⣤⣤⣤⣄⣀⠀⠀⣠⣶⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀", "green", "on_black", ["bold"])
    ...
    print(
        ...
        + colored(
            "⠀⠀⠀⠀⠀⠀⠀⠀⠈⠈⠂⠑⠀⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀[ V-U1.0.1 ]", "light_cyan", "on_black", ["bold"]
        )
    )
    print(
        colored(
...
def select_stream(yt: YouTube, quality: str) -> Optional[Any]:
    filters: Dict[str, Any] = {
        "audio": lambda: yt.streams.filter(only_audio=True)
        .order_by("abr")
        .desc()
        .first(),
...
def quality_input() -> str:
    ...
    quality_choice: str = input(
        colored("        [?] Enter the number of your choice: ", "cyan", attrs=["bold"])
    ).strip()
...
def download_single_video(video_url: str) -> None:
    ...
    try:
        ...
        print(
            colored(
                f"        [i] Downloading video: {yt.title} ({quality})",
                color="blue",
                attrs=["bold"],
            )
        )
        ...
        if stream:
            ...
            print(
                colored(
                    f"        [i] Download complete: {yt.title}",
                    color="green",
                    attrs=["bold"],
                )
            )
            ...
            print(
                colored(
                    f"        [!] No stream found for {yt.title} with quality {quality}",
                    color="red",
                    attrs=["bold"],
                )
            )
...
def playlist_download_video(
    ...
    try:
        ...
        print(
            colored(
                f"        [i] Downloading video ({idx}/{total_videos}): {video.title} ({quality})",
                color="blue",
                attrs=["bold"],
            )
        )
        ...
        if stream:
            ...
            print(
                colored(
                    f"        [i] Download complete: {video.title}",
                    color="green",
                    attrs=["bold"],
                )
            )
            ...
            print(
                colored(
                    f"        [!] No stream found for {video.title} with quality {quality}",
                    color="red",
                    attrs=["bold"],
                )
            )
        ...
        print(
            colored(
                f"        [!] Failed to download {video.title}: {e}",
                color="red",
                attrs=["bold"],
            )
        )
...
def playlist_create_directories(
    playlist_title: str, quality: str
) -> Tuple[str, str, str]:
...
def download_playlist_videos(playlist_url: str) -> None:
    ...
    print(
        colored(
            f"        [i] Downloading playlist: {playlist.title}",
            color="magenta",
            attrs=["bold"],
        )
    )
    quality: str = quality_input()
    playlist_folder, quality_folder, log_folder = playlist_create_directories(
        playlist.title, quality
    )
    ...
    log_data: Dict[str, List[str]] = load_json(log_file) or {
        "completed": [],
        "not_completed": [],
    }
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
            ...
            if not playlist_video_exists(YouTube(video_url), log_data)
        ]
...
def download_channel_video(
    ...
    try:
        ...
        print(
            colored(
                f"        [i] Downloading video ({idx}/{total_videos}): {yt.title} ({quality})",
                color="blue",
                attrs=["bold"],
            )
        )
        ...
        print(
            colored(
                f"        [!] Failed to download {yt.title}: {e} ",
                color="red",
                attrs=["bold"],
            )
        )
...
def channel_get_video_urls(
    api_key: str, channel_id: str, channel_name: str
) -> List[str]:
    base_video_url: str = "https://www.youtube.com/watch?v="
    base_search_url: str = "https://www.googleapis.com/youtube/v3/search?"
    links_file: str = os.path.join(
        BASE_DIR, "channels", channel_name, "download_data", "links.json"
    )
    if os.path.exists(links_file):
        with open(links_file, "r") as f:
            ...
            print(
                colored(
                    "        [i] There is info and links of this channel in local db, not fetching from google:",
                    "cyan",
                    attrs=["bold"],
                )
            )
    ...
    first_url: str = (
        f"{base_search_url}key={api_key}&channelId={channel_id}&part=snippet,id&order=date&maxResults=50"
    )
    video_urls: List[str] = []
    url: str = first_url
    while True:
    ...
    os.makedirs(
        os.path.join(BASE_DIR, "channels", channel_name, "download_data"), exist_ok=True
    )
    with open(links_file, "w") as f:
        json.dump({"video_urls": video_urls}, f, indent=4)
    print(
        colored(
            "        [i] Fetching from google info and links of this channel:",
            "cyan",
            attrs=["bold"],
        )
    )
...
def download_channel(api_key: str, channel_id: str, channel_name: str) -> None:
    ...
    if log_data["completed"] or log_data["not_completed"]:
        print(
            colored(
                "        [i] Some records found for this channel and quality.", "cyan"
            )
        )
        ...
        print(
            colored(
                "        [1] Download only incomplete items from the JSON database.",
                "cyan",
            )
        )
        print(colored("        [2] Recheck all videos from Google data.", "cyan"))
        choice: str = input(
            colored("        [?] Enter your choice (default: 1): ", "blue")
        ).strip()
        if choice == "2":
            ...
            print(
                colored(
                    "        [i] Downloading incomplete items from the JSON database.",
                    "cyan",
                )
            )
    else:
        print(
            colored(
                "        [i] No downloading records found for this channel and quality.",
                "cyan",
            )
        )
        print(
            colored(
                "        [i] Proceeding to fetch video URLs from Google data.", "cyan"
            )
        )
    total_videos: int = len(video_urls)
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for idx, video_url in enumerate(video_urls, start=1):
            if channel_video_exists(video_url, log_data):
                print(
                    colored(
                        f"        [~] Skipping already downloaded video ({idx}/{total_videos}): {video_url} ",
                        color="yellow",
                        attrs=["bold"],
                    )
                )
            ...
            futures.append(
                executor.submit(
                    ...
                    video_url,
                    os.path.join(BASE_DIR, "channels", channel_name, quality),
                    idx,
                    total_videos,
                    log_file,
                    log_data,
...
def start_u_ydt():
    ...
    if choice == "1":
        ...
        playlist_url = input(
            colored("        [~] Enter playlist URL or ID: ", "yellow")
        ).strip()
        normalized_url = normalize_playlist_url(playlist_url)
        playlist_title = handle_playlist_input(log, normalized_url)
        save_log(log)
        download_playlist_videos(normalized_url)
    elif choice == "3":
        single_content_url = input(
            colored("        [~] Enter single content URL or ID: ", "yellow")
        ).strip()
        normalized_url = normalize_video_url(single_content_url)
        video_title = handle_single_content_input(log, normalized_url)
        save_log(log)
...
def handle_channel_input(log):
    if "api_keys" in log and log["api_keys"]:
        api_key = select_api_key(log)
    else:
        api_key = input(colored("        [~] Enter new API key: ", "yellow")).strip()
        log["api_keys"] = [api_key]

    channel_id = input(colored("        [~] Enter channel ID: ", "yellow")).strip()
    if "channels" in log and channel_id in [c["channel_id"] for c in log["channels"]]:
        ...
        print(
            colored(
                f"        [~] Channel exists in log, named: {channel_name}", "yellow"
            )
        )
    else:
        channel_name = input(
            colored("        [~] Enter channel name: ", "yellow")
        ).strip()
...
def handle_playlist_input(log, normalized_url):
    if "playlists" in log and normalized_url in [
        p["playlist_url"] for p in log["playlists"]
    ]:
        playlist = next(
            p for p in log["playlists"] if p["playlist_url"] == normalized_url
        )
        playlist_title = playlist["playlist_title"]
        print(
            colored(
                f"        [~] Playlist exists in log, named: {playlist_title}", "yellow"
            )
        )
    else:
        playlist_title = fetch_playlist_title(normalized_url)
        log.setdefault("playlists", []).append(
            {"playlist_url": normalized_url, "playlist_title": playlist_title}
        )
    return playlist_title


def handle_single_content_input(log, normalized_url):
    if "single_contents" in log and normalized_url in [
        v["video_url"] for v in log["single_contents"]
    ]:
        video = next(
            v for v in log["single_contents"] if v["video_url"] == normalized_url
        )
        video_title = video["video_title"]
        print(
            colored(
                f"        [~] The content exists in log, named: {video_title}", "yellow"
            )
        )
    else:
        video_title = fetch_video_title(normalized_url)
        log.setdefault("single_contents", []).append(
            {"video_url": normalized_url, "video_title": video_title}
        )
    return video_title


def check_previous_downloads(log):
    print(
        colored(
            "        [i] Which type of previous downloads do you want to check?", "blue"
        )
    )
    ...
    check_choice = input(colored("        [~] Choose an option: ", "yellow")).strip()
...
def handle_check_videos(log):
    if "single_contents" in log:
        ...
        video_choice = int(
            input(
                colored(
                    "        [~] Enter the number of the video to download: ", "yellow"
                )
            ).strip()
        )
...
def handle_check_channels(log):
    if "channels" in log:
        ...
        channel_choice = int(
            input(
                colored(
                    "        [~] Enter the number of the channel to download: ",
                    "yellow",
                )
            ).strip()
        )
...
def handle_check_all(log):
    ...
    if "single_contents" in log:
        ...
        for video in log["single_contents"]:
            print(
                colored(
                    f"                [{sn}] {video['video_title']} ({video['video_url']})",
                    "cyan",
                )
            )
            sn += 1
    if "playlists" in log:
        ...
        for playlist in log["playlists"]:
            print(
                colored(
                    f"                [{sn}] {playlist['playlist_title']} ({playlist['playlist_url']})",
                    "cyan",
                )
            )
            sn += 1
    if "channels" in log:
        ...
        for channel in log["channels"]:
            print(
                colored(
                    f"                [{sn}] {channel['channel_name']} ({channel['channel_id']})",
                    "cyan",
                )
            )
            sn += 1
    prev_choice = int(
        input(
            colored(
                "        [~] Enter the number of the download to complete: ", "yellow"
            )
        ).strip()
    )
    ...
    if prev_choice <= len(log.get("single_contents", [])):
    ...
    elif prev_choice <= len(log.get("single_contents", [])) + len(
        log.get("playlists", [])
    ):
        download_playlist_videos(
            log["playlists"][prev_choice - len(log.get("single_contents", [])) - 1][
                "playlist_url"
            ]
        )
    else:
        index = (
            prev_choice
            - len(log.get("single_contents", []))
            - len(log.get("playlists", []))
            - 1
        )
        ...
        save_log(log)
        download_channel(
            api_key, channel_info["channel_id"], channel_info["channel_name"]
        )
